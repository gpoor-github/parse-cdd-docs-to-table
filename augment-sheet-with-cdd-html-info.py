...
def read_table(file_name: str):
    ...
    with open(file_name) as csv_file:
        ...
        print("End for loop")
        return table, keys_from_table, header
...
def write_sheet(file_name: str, table: [], keys_to_find_and_write, keys_for_section_data, key_to_java_objects, key_to_urls,
    ...
    with open(file_name, 'w', newline='') as csv_output_file:
        ...
        for temp_key in keys_to_find_and_write:
            ...
            if section_data:
                found_count += 1
                table[output_count].append('{}'.format(section_data))
                ...
                table[output_count].append(f'! ERROR {key_str}: {notfound_count} Not found in data keys.')
                table[output_count].append(key_to_urls.get(key_str))

            table[output_count].append(key_to_java_objects.get(key_str))
            if keys_to_files_dict:
                filename = keys_to_files_dict.get(key_str)

                if filename:
                    table[output_count].append(filename)
                    ...
                    if len(class_name_split_src) > 1:
                        ...
                        table[output_count][7] = class_name
                        table[output_count][3] = "Yes"
            table[output_count].append('Last augmented column')
...
class AugmentSheetWithCDDInfo:
    ...
    def augment_table(self):
        ...
        table1, keys_from_table, key_to_full_requirement_text, key_to_java_objects, key_to_urls, keys_not_found, cdd_string = \
            self.parse_cdd_html_to_requirements_table('new_recs_todo.csv',
                                                      'Android 11 Compatibility Definition_full_original.html')

        keys_to_files_dict = None
        # keys_to_files_dict = find_test_files(key_to_java_objects)

        keys_not_found = write_sheet('augmented_output.csv', table1, keys_from_table, key_to_full_requirement_text,
