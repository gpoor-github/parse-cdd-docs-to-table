...
def write_new_data_line_to_table(key_str: str, keys_to_sections: dict, table: [[str]], table_row_index: int):
    ...
    table[table_row_index][static_data.new_header.index(static_data.SECTION_ID)] = key_split[0]
...
def cdd_html_to_cts_create_sheets(targets: str = 'all',
    ...
    write_table(created_table_file, table_for_sheet, static_data.new_header)
    # else:
...
if __name__ == '__main__':
    ...
    rd.do_search().pipe(rd.get_pipe_create_results_table()).subscribe(
        on_next=lambda table: table_ops.write_table("output/rx_try13_just_todo.csv", table, static_data.new_header),
